...
public class Minvo extends Mob {
    ...
    private Map map;
    public static double SPEED = 3.2;
    private double speedH;
    private double speedV;
    private Player player;

    public Minvo(int x, int y, Map map) {
        if (balloomImg == null) {
            ...
            try {
                for (int i = 1; i <= 10; i++) {
                    ...
                    balloomImg.add(new Image(getClass()
                            .getResource("/image/mobs/minvo/" + i + ".png").toURI().toString()));
                }
    ...
    public void moveEnemy() {
        ...
        setX(abs(getX() + speedH));
        setY(abs(getY() + speedV));
    }

    double abs(double so) {
        double ct = (int) (so);
        if (ct + 0.5 <= so) {
            ct += 1;
        }
